  ...
  <div id="main" class="main">
    ...
    <div class="contbox" :style="divMargin" @click="tabflag = false">
      <div class="cont-inner" :style="innerStyle">
        ...
        <div class="titlebox">
          <img src="http://res.jinpeiwang.cn/gs_bx/xueru/board/web/ttl.png" />
          <!-- <div class="title">
        ...
        <div class="topcont">
          <div class="todaydata">
            <div class="today-inner">
              ...
              <henan-chart :dom="'mychart1'" :data="yjtrend" :name="'金额（万元）'" ref="chart1"></henan-chart>
              <!-- <div class="cm-color-white cm-font-size-20">
          ...
          <div class="chartbox">
            <div class="chartbox-inner">
              <div class="cm-color-white cm-font-size-20 cm-padded-b-20">
                <span class="data-title info-title">河南邮政邮银业绩总额</span>
              </div>
          ...
          <div class="databox">
            <div class="cm-color-white cm-font-size-20 cm-padded-b-15">
              <span class="info-title">大单喜报</span>
            </div>
            <swiper :options="swiperOption1" v-if="bulletinList.length > 0" class="swiperbox cm-padded-20">
              <swiper-slide class="slide" v-for="(item, key) in bulletinList" :key="key">
                <div class="slidebox" :class="'slide' + item.num" v-if="item.type !== '0'">
                  <div v-if="item.type == '1'" :class="`slide${item.num}`">
                    <img :src="`http://res.jinpeiwang.cn/gs_bx/xueru/henan/image/xb/${item.num}.png`">
                  ...
                  <div v-else-if="item.type == '2'" :class="`slide${item.num}`">
                    <img :src="`http://res.jinpeiwang.cn/gs_bx/xueru/henan/image/xb/xb${item.num}.png`">
                    <div class="slideinner">
                      <div class="avatarbox" :class="`avatarbox${item.num}`">
                        <img :src="`http://res.jinpeiwang.cn/gs_bx/xueru/henan/image/xb/avatarbg${item.num}.png`" class="avatarbg" alt="">
                        <div class="avatardiv cm-f-c-c" :class="`avatardiv${item.num}`">
                          <img :src="item.photo_address" alt="">
                        </div>
                      </div>
                      <div class="namebgbox" v-if="item.num == 2">
                        <img src="http://res.jinpeiwang.cn/gs_bx/xueru/henan/image/xb/namebg.png" alt="">
                        <div class="username cm-tx-c" v-text="item.name">张三</div>
                      </div>
                      <img src="http://res.jinpeiwang.cn/gs_bx/xueru/henan/image/xb/xbinfo.png" class="xbinfo" v-if="item.num == 1" alt="">
                      <div v-if="item.num == 1" class="xbuserinfo" :class="`xbuserinfo${item.num}`">
                        <div class="prov">
                          <div v-text="`热烈庆祝${item.province}`"></div>
                        </div>
                        <div class="cm-f-c-c">
                          <div class="name cm-margin-r-10" v-text="item.name">张三</div>
                          <div class="amount" v-text="item.fee">50万元</div>
                        </div>
                      </div>
                      <div v-if="item.num == 2" class="xbuserinfo" :class="`xbuserinfo${item.num}`">
                        <div class="prov">
                          <div v-text="`热烈庆祝${item.province}`"></div>
                        </div>
                        <div class="amount" v-text="item.fee">50万元</div>
                      </div>
                      <div class="product xbproduct" v-text="item.product_name">富德生命金典人生终身寿险</div>
                    </div>
                  </div>
                  <div v-else>
        ...
        <div class="botcont cm-f-b-e">
          <div class="leftcont">
            <div>
              <div class="info-title cm-color-white cm-margin-b-20">河南邮政各地市业绩排名（万元）</div>
              <div class="listbox bg-white cm-font-size-14" style="min-height: 540px;">
                <div class="list list36 cm-f-b-c bg-dark-red">
                  <div class="item width18 cm-color-white border-r">序号</div>
                  <div class="item width36 cm-color-white border-r">地市</div>
          ...
          <div class="leftcont">
            <div class="info-title cm-color-white cm-margin-b-20">保险公司总业绩排名（万元）</div>
            <div class="listbox bg-white cm-font-size-14" style="min-height: 540px;">
              <div class="list list36 cm-f-b-c bg-dark-red">
                <div class="item width18 cm-color-white border-r">序号</div>
          ...
          <!-- <div class="midcont1">
            <div class="info-title cm-color-white cm-margin-b-20">各省自营总业绩排名（万元）<span class="cm-margin-l-5 cm-font-size-12">每小时一更新</span></div>
          ...
          <div class="rightcont">
            <!-- <div class="info-title cm-color-white cm-margin-b-20">保险公司总业绩排名（万元）<span class="cm-margin-l-5 cm-font-size-12">每小时一更新</span></div>
            <div class="listbox bg-white cm-font-size-14">
              <div class="list list36 cm-f-b-c bg-dark-red">
                <div class="item width18 cm-color-white border-r">序号</div>
                <div class="item width36 cm-color-white border-r">保险公司</div>
            ...
            <div class="info-title cm-color-white margin-t-40 cm-margin-b-20 cm-f-s-c" @click.stop="openSelect">
              <div>
                <span v-text="companyTitle"></span><span>各地市业绩排名（万元）</span>
              </div>
            ...
            <div class="listbox bg-white cm-font-size-14" style="min-height: 540px;">
              <div class="list bg-dark-red list36 cm-f-b-c">
                ...
                <div class="item width24 cm-color-white border-r">地市</div>
              ...
              <swiper class="swiper2" v-if="proflag" :options="swiperOption" ref="mySwiper6">
...
<script>
import HenanChart from './HenanChart'
export default {
  name: 'board',
  data () {
    return {
  ...
  created () {
    ...
    this.getProAmount()
    // 保险公司
    this.$get('www', '/Gsbx-bulletin-productCompanyList?pid=2').then((res) => {
    ...
    let title = '河南邮政2021代理保险旺季营销业绩展板'
    let desc = '比学赶帮超，豫邮争一流！2021河南邮政加油！'
    let link = 'http://www.jinpeiwang.cn/Public/henanboard/#/board'
    let sharelink = location.href + '?&'
    let imgUrl = 'http://res.jinpeiwang.cn/gs_bx/xueru/henan/image/henanshare.png'
  ...
  methods: {
    ...
    getGroupAmount () {
      // 获取业绩总额
      this.$get('www', '/Gsbx-bulletin-groupAmount?pid=2').then((res) => {
        // console.log(JSON.stringify(res))
        if (res) {
          if (res.data.status === 'ok') {
    ...
    getGroupAmountTrend () {
      // 业绩趋势
      this.$get('www', '/Gsbx-bulletin-groupAmountTrend?pid=2').then((res) => {
        // console.log(JSON.stringify(res))
        if (res) {
          if (res.data.status === 'ok') {
            this.yjtrend.time = []
    ...
    getProvinceYYAmount () {
      // 各省邮银
      this.$get('www', '/Gsbx-bulletin-provinceYYAmount?pid=2').then((res) => {
        // console.log(JSON.stringify(res))
        if (res) {
          if (res.data.status === 'ok') {
    ...
    getCompanyAmount () {
      // 保险公司总业绩
      this.$get('www', '/Gsbx-bulletin-companyAmount?pid=2').then((res) => {
        // console.log(JSON.stringify(res))
        if (res) {
          if (res.data.status === 'ok') {
            ...
            this.companyAmount = this.setListData(arrData, 18)
          }
        }
      })
    },
    getBulletinList () {
      // 获取大单喜报
      this.$get('www', '/Gsbx-bulletin-bulletinList?pid=2').then((res) => {
        // console.log(JSON.stringify(res))
        if (res) {
          if (res.data.status === 'ok') {
            // var n = 1
            ...
            //   }
            //   if (obj.fee >= 10000) {
            //     obj.fee = (obj.fee * 0.0001).toFixed(1) + '万元'
            //   } else if (obj.fee >= 1000) {
            //     // obj.fee = obj.fee + '元'
            //     obj.fee = (obj.fee * 0.0001).toFixed(1) + '万元'
            //   } else {
            //     obj.fee = (obj.fee * 0.0001).toFixed(2) + '万元'
            //   }
            //   return obj
            // })
            var n = 1
            var p = 1
            var ddxbList = res.data.msg.ddxb.map(data => {
              var obj = data
              if (!data.address) {
                if (!data.photo_address) {
                  obj.num = n
                  obj.type = '1'
                  obj.address = 'https://jinpeiwang.oss-cn-beijing.aliyuncs.com/gs_bx/xueru/henan/image/xb/' + n + '.png'
                  n += 1
                  if (n > 3) {
                    n = 1
                  }
                } else if (data.photo_address) {
                  obj.num = p
                  obj.type = '2'
                  obj.address = 'https://jinpeiwang.oss-cn-beijing.aliyuncs.com/gs_bx/xueru/henan/image/xb/xb' + p + '.png'
                  p += 1
                  if (p > 2) {
                    p = 1
                  }
            ...
            var i = 1
            var pyList = res.data.msg.py.map(data => {
              var obj = data
              obj.type = '0'
              if (!data.address) {
                obj.num = i
                obj.type = '1'
                obj.address = 'https://jinpeiwang.oss-cn-beijing.aliyuncs.com/gs_bx/xueru/henan/image/xb/' + i + '.png'
                i += 1
                if (i > 3) {
                  i = 1
                }
            ...
            this.bulletinList = ddxbList.concat(pyList)
          }
        }
      })
    },
    ...
    getProAmount () {
      var url
      if (this.cid) {
        url = '/Gsbx-bulletin-companyProAmount?pid=2&cid=' + this.cid
      } else {
        url = '/Gsbx-bulletin-companyProAmount?pid=2'
      }
      this.proflag = false
      this.$get('www', url).then((res) => {
        ...
        if (res) {
          if (res.data.status === 'ok') {
            ...
            this.companyProAmount = this.setListData(arrData, 18)
            this.proflag = true
...
<style scoped>
@import url('../../assets/css/henan.css');
</style>
